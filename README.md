...
> A WorkBuddy Skill that turns a folder of mixed-format study notes (Word / PDF / images / PPT / audio & video recordings) into organized, per-month, section-classified revision-summary Word documents — using only free local tools, no paid API.
...
- **Multi-format parsing**: `.docx` / `.doc` / `.pdf` (text, scanned **and** watermark-overlay slide decks) / `.pptx` / `.xlsx` / images / **audio & video recordings**, plus `.zip` / `.rar` archives (auto-extracted and recursed), all unified into Markdown.
- **Speech-to-text for recordings**: `.mp3` / `.m4a` / `.wav` / `.flac` / `.ogg` / `.aac` and video (`.mp4` / `.mov` / `.webm` / `.m4v`) are transcribed locally with **faster-whisper** — free, CPU-only, no cloud API, language auto-detected.
- **Fully free local engine**: `python-docx`, `macOS textutil`, `pdfplumber`, `python-pptx`, `openpyxl`, `tesseract OCR`, `img2pdf`, `rarfile`, `faster-whisper` (PyAV bundles its own ffmpeg) — no network, no per-use billing.
...
- **OCR engine `tesseract`: auto-installed when missing.** Windows→`winget install UB-Mannheim.TesseractOCR`; macOS→`brew install tesseract` (Linux/apt needs sudo and prints a manual hint). Manual: `winget install UB-Mannheim.TesseractOCR`. Disable with `NOTES_SKIP_TESSERACT_INSTALL=1`.
  Note: German B1 notes are all text-based and never trigger OCR; OCR only applies to scans, photos, and watermark-overlay slide-deck PDFs (text layer is mostly repeated header/watermark lines while the real content lives in page images — auto-detected and re-OCR'd whole-file).
...
- **Image / scanned-PDF OCR**:
  ...
  - Default OCR language `chi_sim+deu+eng` (Chinese+German+English, subject-agnostic); missing packs **auto-download** to `~/.notes_ocr_tessdata` on first run, override via `NOTES_OCR_LANG` env var. Install all packs offline with `brew install tesseract-lang` (macOS).
- **Speech-to-text for audio/video (zero cost, fully local)**: `.mp3` / `.m4a` / `.wav` / `.flac` / `.ogg` / `.aac` and video containers (`.mp4` / `.mov` / `.webm` / `.m4v`) are transcribed with **faster-whisper** (open-source, CPU, no cloud). Audio decoding uses **PyAV**, which **bundles its own ffmpeg**, so no separate ffmpeg install is needed. The Whisper model **auto-downloads from HuggingFace on first use** and is cached; default model `small`. Tune via `NOTES_WHISPER_MODEL=base` (faster) or `medium` (best), and `NOTES_WHISPER_LANG=zh`/`en` to force a language.
...
- **Default OCR language `chi_sim+deu+eng`** (Chinese+German+English, subject-agnostic). Override via env var: `NOTES_OCR_LANG=eng` for English-only, etc.; missing packs auto-download on first run.
- **Scan / handwriting image quality** determines OCR quality; clean print is best, sloppy handwriting may need human proofreading.
- **Audio/video recordings** are transcribed locally with Whisper (no cloud, no paid API). The first transcription downloads the model from HuggingFace (needs internet once); CPU-only is slower than GPU but works everywhere. For clearer Chinese lectures, set `NOTES_WHISPER_LANG=zh` or use `NOTES_WHISPER_MODEL=medium`.
